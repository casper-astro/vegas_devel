...
int main(int argc, char *argv[]) {
    ...
    /* Data buffer ids */
    struct guppi_thread_args net_args, dedisp_args, disk_args;
    ...
    dbuf_fold = guppi_databuf_attach(dedisp_args.output_buffer);
    if (dbuf_fold==NULL) {
        fprintf(stderr, "Error connecting to guppi_databuf\n");
        exit(1);
    }
