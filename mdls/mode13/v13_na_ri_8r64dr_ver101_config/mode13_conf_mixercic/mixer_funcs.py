...
def set_gain_sg(fpgaclient, gain):
    fpgaclient.write('s1_quant_gain', gain)
...
def read_cic_snaps(fpga, dec_rate, snap_depth):
    fpga.write_int('s1_cic1_out_ctrl', 0)
    fpga.write_int('s1_hb_out_ctrl', 0)
    fpga.write_int('s1_cic2_out_ctrl', 0)
    time.sleep(3)

    fpga.write_int('s1_cic1_out_ctrl', 7)
    fpga.write_int('s1_hb_out_ctrl', 7)
    fpga.write_int('s1_cic2_out_ctrl', 7)
    time.sleep(3)
    ...
    cic1_m = struct.unpack('>'+str(2**snap_depth)+'l',fpga.read('s1_cic1_out_bram_msb', (2**snap_depth)*4))
    cic1_l = struct.unpack('>'+str(2**snap_depth)+'L',fpga.read('s1_cic1_out_bram_lsb', (2**snap_depth)*4))
    hb_m = struct.unpack('>'+str(2**snap_depth)+'l',fpga.read('s1_hb_out_bram_msb', (2**snap_depth)*4))
    hb_l = struct.unpack('>'+str(2**snap_depth)+'L',fpga.read('s1_hb_out_bram_lsb', (2**snap_depth)*4))
    cic2_m = struct.unpack('>'+str(2**snap_depth)+'l',fpga.read('s1_cic2_out_bram_msb', (2**snap_depth)*4))
    cic2_l = struct.unpack('>'+str(2**snap_depth)+'L',fpga.read('s1_cic2_out_bram_lsb', (2**snap_depth)*4))
