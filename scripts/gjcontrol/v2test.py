"""
v2test.py - convenience routines for running Vegas mode 1 on roach 2 testing at Green Bank
--------
...
# custom subclass to fix broken write_int method
class R2Client(katcp_wrapper.FpgaClient):
    def write(self,device_name, data, offset=0):
        raise Exception("writes are currently broken")
    def write_int(self, device_name, integer, blindwrite=False, offset=0):
        self._request("wordwrite",device_name,"0","0x%08x" % integer)
...
for rn in range(1,9):
    ...
    if rv not in globals():
        try:
            ...
            if rn == 9:
                roach = R2Client('vegasr2-test')
            else:   
                roach = R2Client('vegasr2-%d' % rn)
            tic = time.time()
...
mac_base = (2 << 40) + (2<<32)
fabric_port = 60000
def setupNet(rns = range(1,9),starttap=True,loc=False):
    """
...
def watch(rns = range(1,9)):
    ...
    while True:
        for rn in rns:
            try:
                roach = roachd[rn]
                sync[rn] = roach.read_int('sg_sync')
                status[rn] = roach.read_int('status')
                arm[rn] = roach.read_int('arm')
            except:
                pass
        print "At:",time.ctime()
        for k in rns:
            if status.has_key(k):
                print "Roach: %d Status 0x%04x Sync 0x%04x Arm %d" % (k,status[k],sync[k],arm[k])
        sys.stdout.flush()
        time.sleep(0.1)
...
def reset(rns = range(1,9),acc_len=768):
    ...
    for rn in rns:
        ...
        roach.write_int('sg_period',acc_len*128*7*1024-2) # found that -4 provides proper TIMECNTR increment
...
def getAdcSnap(roach):
    ...
    roach.write_int('adcsnap0_ctrl',0)
    roach.write_int('adcsnap1_ctrl',0)    
    roach.write_int('adcsnap0_ctrl',1)
    roach.write_int('adcsnap1_ctrl',1)    
    roach.write_int('raw_adc_trig',0xFFFFFFFF)
    x = np.fromstring(roach.read('adcsnap0_bram',16384),dtype='int8')
    y = np.fromstring(roach.read('adcsnap1_bram',16384),dtype='int8')
    return x,y
    
def getStatus(roach):
    s = roach.read_int('sg_sync')
    d = roach.read_int('status')
    print "sg_sync= 0x%02X" % s
    if s & 0x10 == 0:
        print "*** sg_sync must have bit 0x10 set ***"
    if s & 0x02:
        print "*** sync generator disabled"
    if s & 0x04:
        print "external PPS enabled ",
        if d & 0x10:
            print "and armed"
        else:  
            print "but NOT armed"
    else:
        print "external PPS disabled"
...
#        print "*** Link down ***"
    if d & 0x04:
        print "*** TGE in reset ***"
    if d & 0x08:
        print "*** Master rst asserted ***"
    if d & 0x10:
        print "*** Spead rst asserted ***"
    if d & 0x20:
        print "data enabled"
    else:
        print "*** data disabled (not synced) ***"
    print "reshape state is:", (d>>6)&0x7
    if d & 0x200:
        print "data valid"
    print "spead state is:", (d>>10)&0x7

def getAdcSnapR1(roach):
    """
...
def mansync(r):
    r.write_int('sg_sync',0x12)
    r.write_int('arm',0)
    r.write_int('arm',1)
    r.write_int('arm',0)
    r.write_int('sg_sync',0x11)
    

def sync(rns=range(1,9),loc=False):
    """
    ...
    print "Preparing"
    for rn in rns:
        roachd[rn].write_int('debug_ctrl',4) # prep debug snapshot
        roachd[rn].write_int('debug_ctrl',5) # prep debug snapshot
...
Xcfs = Xlo1 - (lo2+720.0-10500.0)
Ccfs = Clo1 - (lo2+720.0-10500.0)
...
def Reinit(nodes=range(1,9)):
    ...
    * starts VegasServer
    ...
    for k in nodes:
        ...
        print "dbuf1  ",k
        cmd = "$VEGAS_DIR/bin/check_guppi_databuf -c -i1 -n2 -s32768 -t1 &> $VEGAS/logs/check_guppi_databuf1.%s.log" % str(k)
        print cmd
        remoteExec(k,cmd)
        print "dbuf2  ",k
        cmd = "$VEGAS_DIR/bin/check_guppi_databuf -c -i2 -n8 -s256 -t2 &> $VEGAS/logs/check_guppi_databuf2.%s.log" % str(k)
        print cmd
        remoteExec(k,cmd)
        print "dbuf3  ",k
        cmd = "$VEGAS_DIR/bin/check_guppi_databuf -c -i3 -n8 -s256 -t2 &> $VEGAS/logs/check_guppi_databuf3.%s.log" % str(k)
        print cmd
        remoteExec(k,cmd)
...
def plotDebug(r1,man_trig=False,man_valid=False):
    f = plt.figure()
    ax = f.add_subplot(111)
    d = np.fromstring(r1.read('debug_bram',16384*4),dtype='>i4')
    nbits = [1,1,1,1,1,1, 3, 1, 3, 1, 1, 1, 1]
    labels = ['txovf','valid','tge_rst','master_rst','spead_rst','data_enable','rshp_st','dvalid_spead','spead_st','eof','eoh','dvalid','rshp_rst']
#    nbits = [4,1,1,1,1,1,1,1,1]
    offs = 0
    yoffs = 0
    scale = 1
    trans = plt.matplotlib.transforms.blended_transform_factory(
            ax.transAxes, ax.transData)
    for val,label in zip(nbits,labels):
        ax.plot(((d>>offs) & (2**val - 1))+yoffs*scale, drawstyle='steps',linewidth=1.5,color='r')
#        print yoffs, yoffs*scale,yoffs*scale+2**val-1
        ax.axhline(yoffs*scale,alpha=0.5)
        ax.text(0.1,yoffs*scale + (2**val-1)/2.0, label,fontsize='xx-small',ha='left',va='center', transform = trans)
        ax.axhline(yoffs*scale+2**val-1,alpha=0.5)
        offs += val
        yoffs += 2**val -1 + 0.3
