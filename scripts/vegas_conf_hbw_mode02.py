...
time.sleep(1)
'''
    formula for computing sync period:
    n_accumulation * LCM(orders)* n_taps_pfbfir * NChan/ num_si_inputs
    n_accumulation = acc_len
    orders = 2 and 8 (for this FFT), so LCM(orders) = 8
    n_taps for pfb_fir = 4
    NChan = 4096 (2k channel real)
    number of simultaneous inputs = 16
    so sg_period in this case =
        acc_len * 8 * 4 * (4096/16)
'''
fpga.write_int('sg_period', acc_len*8*4*256-4)
