 ...
 */
extern "C"
int init_gpu(int instance_id, size_t input_block_sz, size_t output_block_sz, int num_subbands, int num_chans)
{
    int iDevCount = 0;
    ...
    else if (1 == iDevCount)    /* check for at least two GPUs */
    {
        if (instance_id != 0)   /* if instance 0, no problem */
        {
            (void) fprintf(stderr,
                           "ERROR: Not enough CUDA-capable devices!\n");
            run = 0;
            return EXIT_FAILURE;
        }
    }
    ...
    /* sanity check */
    if (instance_id >= iDevCount)
    {
        (void) fprintf(stderr,
                       "ERROR: Instance ID inconsistent with number of "
                       "CUDA-capable devices!\n");
        run = 0;
        return EXIT_FAILURE;
    }

    /* use the device corresponding to the instance_id (which can only be 0 or 1) */
    CUDASafeCall(cudaSetDevice(instance_id));
