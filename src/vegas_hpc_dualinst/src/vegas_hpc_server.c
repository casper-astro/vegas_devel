...
void init_hbw_mode(struct vegas_thread_args *args, int *nthread, int instance_id) {
    vegas_thread_args_init(&args[0], instance_id); // net
    vegas_thread_args_init(&args[1], instance_id); // accum
...
void init_lbw_mode(struct vegas_thread_args *args, int *nthread, int instance_id) {
    vegas_thread_args_init(&args[0], instance_id); // net
    vegas_thread_args_init(&args[1], instance_id); // pfb
    vegas_thread_args_init(&args[2], instance_id); // accum
...
void init_monitor_mode(struct vegas_thread_args *args, int *nthread, int instance_id) {
    vegas_thread_args_init(&args[0], instance_id); // net
    vegas_thread_args_init(&args[1], instance_id); // null
    args[0].output_buffer = 1;
    args[1].input_buffer = args[0].output_buffer;
    *nthread = 2;
}
...
#define MAX_CMD_LEN 1024
    ...
    rv = vegas_status_attach(instance_id, &stat);
    ...
    dbuf_net = vegas_databuf_attach(instance_id, netbuf_id);
    if (dbuf_net==NULL) {
        fprintf(stderr, "Error connecting to vegas_databuf (raw net)\n");
        exit(1);
    }
    vegas_databuf_clear(dbuf_net);
    dbuf_pfb = vegas_databuf_attach(instance_id, pfbbuf_id);
    if (dbuf_pfb==NULL) {
    ...
    dbuf_acc = vegas_databuf_attach(instance_id, accbuf_id);
    if (dbuf_acc==NULL) {
