...
int main(int argc, char *argv[]) {
    ...
    while ((opt=getopt_long(argc,argv,"hdoI:",long_opts,&opti))!=-1) {
        switch (opt) {
            case 'I':
    ...
    dbuf = vegas_databuf_attach(instance_id, net_args.output_buffer);
    /* If attach fails, first try to create the databuf */
    if (dbuf==NULL) 
#ifdef NEW_GBT
        dbuf = vegas_databuf_create(instance_id, 24, 32*1024*1024, net_args.output_buffer, CPU_INPUT_BUF);
#else
    ...
    vegas_thread_args_init(&null_args, instance_id);
