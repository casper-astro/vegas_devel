...
void vegas_pfb_thread(void *_args) {
    ...
    db_in = vegas_databuf_attach(instance_id, args->input_buffer);
    if (db_in==NULL) {
        char msg[256];
    ...
    db_out = vegas_databuf_attach(instance_id, args->output_buffer);
    if (db_out==NULL) {
        char msg[256];
        sprintf(msg, "Error attaching to databuf(%d) shared memory.",
                args->output_buffer);
    ...
    vegas_status_unlock_safe(&st);
    if (EXIT_SUCCESS != init_gpu(instance_id,
                                 db_in->block_size,
                                 db_out->block_size,
                                 nsubband,
                                 nchan))
