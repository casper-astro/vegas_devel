...
struct vegas_databuf *vegas_databuf_create(int instance_id, int n_block, size_t block_size,
    ...
    key_t key = vegas_databuf_key(instance_id);
    if(key == VEGAS_KEY_ERROR) {
        vegas_error(__FUNCTION__, "guppi_databuf_key error");
        return(NULL);
    }
    int shmid;
    shmid = shmget(key + databuf_id - 1, 
    ...
    if(buf_type == DISK_INPUT_BUF)
        d->semid = semget(key + databuf_id - 1, MAX_BLKS_PER_BUF, 0666 | IPC_CREAT);
    else
...
struct vegas_databuf *vegas_databuf_attach(int instance_id, int databuf_id) {

    /* Get shmid */
    key_t key = vegas_databuf_key(instance_id);
    if(key == VEGAS_KEY_ERROR) {
        vegas_error("vegas_databuf_attach", "vegas_databuf_key error");
        return(NULL);
    }
    int shmid;
