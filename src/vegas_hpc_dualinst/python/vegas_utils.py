...
def vegas_ipckey(proj_id):
    keyfile = os.getenv('VEGAS_KEYFILE')
    if keyfile == None:
...
class vegas_status:
    ...
    def __init__(self, instance_id=0):
        self.stat_buf = shm.SharedMemoryHandle(vegas_status.ipckey(instance_id))
        self.sem = possem.sem_open(vegas_status.semname(instance_id), possem.O_CREAT, 00644, 1)
        self.hdr = None
        self.gbtstat = None
        self.read()
...
class vegas_databuf:
    ...
    def __init__(self,instance_id=0,databuf_id=1):
        self.buf = shm.SharedMemoryHandle(vegas_databuf.ipckey(instance_id)+databuf_id-1)
    ...
    @staticmethod
    def ipckey(instance_id):
        databuf_key = os.getenv("VEGAS_DATABUF_KEY")
        if databuf_key:
            key = int(databuf_key, 0)
        else:
