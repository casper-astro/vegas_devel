...
void fake_init_block(struct fake_datablock_stats *d, struct vegas_databuf *db, 
...
void write_fake_heap_to_block(struct fake_datablock_stats *d, int heap_cntr)
{
    ...
    struct databuf_index* index = (struct databuf_index*)vegas_databuf_index(d->db, d->block_idx);
    ...
    char *heap_addr = vegas_databuf_data(d->db, d->block_idx) + block_heap_idx*d->heap_size;
 ...
 */
void *vegas_fake_net_thread(void *_args) {
    ...
    if (rv<0) { 
        vegas_error("vegas_fake_net_thread", "Error setting cpu affinity.");
    ...
    if (rv<0) {
        vegas_error("vegas_fake_net_thread", "Error setting priority level.");
        perror("set_priority");
    }
    ...
    if (rv!=VEGAS_OK) {
        vegas_error("vegas_fake_net_thread", 
