...
def send_spead_heap(header, heap):
    ...
    for pkt_num in range(num_pkts):
        ...
        if pkt_num == 0:
            packet += struct.pack('> BHxL BHxL BHxL BHxL BHxL BHxL',
                    (time_cntr_id >> 16) & 0xFF, time_cntr_id & 0xFFFF,
                        heap[time_cntr_id],
                    (spectrum_cntr_id >> 16) & 0xFF, spectrum_cntr_id & 0xFFFF,
                        heap[spectrum_cntr_id],
                    (integ_size_id >> 16) & 0xFF, integ_size_id & 0xFFFF,
                        heap[integ_size_id],
...
for heap_cntr in range(1500, 12000):
    ...
    if (heap_cntr %2) == 0:
       spead_heap[status_bits_id] = 0
    else:
       spead_heap[status_bits_id] = 1
