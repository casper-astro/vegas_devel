...
% coeff_group_arr = [[1,2,1,3]; [1,2,3,4]; [1,2,3,4]];
...
elseif(length(optional_delay_arr) ~= FFTSize)
    strError = strcat('the array ''optional_delay_array'' must be FFTSize elements long;  length(optional_delay_array) = ', num2str(length(optional_relay_array)));
    throwError(strError);
...
for(stageNum = 1:FFTSize)
    ...
    blockName = strcat('bulk_delay_', num2str(stageNum));
    bBulkDelay = xBlock(struct('source',str2func('bulk_delay_draw'), ...
        'name', blockName), ...
        {length(sDelayIn_stripped),coeff_delay_arr(stageNum)}, sDelayIn_stripped, sCoeffSyncStage_stripped);
