function fft_biplex_4x_draw(FFTSize,coeff_bit_width,input_bit_width,inter_stage_bit_width, output_bit_width, shift_arr,max_distro_size_coeff,max_distro_size_delay,register_coeffs,stage_pre_delay,stage_mid_delay,stage_post_delay,bram_latency_fft,bram_latency_unscr,unscr_din_latency)
% %
% %
% xBlock;
% 
...
if((~isInt([FFTSize, input_bit_width, max_distro_size_coeff, max_distro_size_delay, stage_pre_delay ...
    ...
    throwError(strError);
elseif( ~isInt(inter_stage_bit_width) || (~isInt(coeff_bit_width)) ||  (~isInt(shift_arr)))
    strError = 'The following parameter arrays must be composed entirely of integers: optional_delay_arr, coeff_bit_width, shift_arr';
    throwError(strError);
...
elseif(((min(inter_stage_bit_width) < 0))|| max(inter_stage_bit_width) > 24)
    strError = strcat('inter_stage_bit_width must composed of positive integers no greater than 24; inter_stage_bit_width = ', num2str(inter_stage_bit_width));
    throwError(strError);
...
blockTemp = xBlock(struct('source', str2func('biplex_coeff_muxsel_gen_draw'), 'name','biplex_muxsel_gen'), {FFTSize, coeff_bit_width, register_coeffs, max_distro_size_coeff, optional_delay_arr, stage_delay_arr}, {iSync}, {sCoeffAll,sSelAll,sSyncCoreOut});
...
for(i=1:FFTSize)
    ...
    if(i==1)
        ...
        blockTemp = xBlock(struct('source', str2func('biplex_stage_n_draw'), 'name',blockName), {FFTSize, i, input_bit_width, coeff_bit_width, inter_stage_bit_width(i), delays_bram, bram_latency_fft, shift_arr(i), optional_delay_arr(i,:)});
    ...
    elseif (i==2)
        ...
        blockName = strcat('stage_',num2str(i));
        blockTemp = xBlock(struct('source', str2func('biplex_stage_n_draw'), 'name',blockName), {FFTSize, i, inter_stage_bit_width(i-1), coeff_bit_width, inter_stage_bit_width(i), delays_bram, bram_latency_fft, shift_arr(i), optional_delay_arr(i,:)}, {sApbwOutStage{i-1},sAmbwOutStage{i-1}, sSyncOutStage{i-1}, sSelStage{i}}, {sApbwOutStage{i},sAmbwOutStage{i},sSyncOutStage{i}});
    else
        ...
        blockName = strcat('stage_',num2str(i));
        blockTemp = xBlock(struct('source', str2func('biplex_stage_n_draw'), 'name',strcat('stage_',num2str(i))), {FFTSize, i, inter_stage_bit_width(i-1), coeff_bit_width(i-2), inter_stage_bit_width(i), delays_bram, bram_latency_fft, shift_arr(i), optional_delay_arr(i,:)}, {sApbwOutStage{i-1},sAmbwOutStage{i-1}, sSyncOutStage{i-1}, sCoeffStage{i}, sSelStage{i}}, {sApbwOutStage{i},sAmbwOutStage{i},sSyncOutStage{i}});
    end
