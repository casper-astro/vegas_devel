...
function adder_tree_lib_init_xblock_old(blk, n_inputs, latency, first_stage_hdl, behavioral)
add_latency = latency;
	...
	while (1)
		...
		elseif length(stage_outports) <= 4
			% base case, manually instantiate the 2 required adders
				...
				adder2_config.source = 'simd_add_dsp48e_init';
				adder2_config.name = ['adder2_', num2str(n_simd_adders)];
				adder2 = xBlock( adder2_config, {[blk,'/', adder2_config.name], 'Addition', 18 + stage_ind, 17, 18 + stage_ind, ...
							17, 'on', 18, 17, 'Truncate', ...
							'Wrap', 0}, ...
