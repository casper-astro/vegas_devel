...
if n_inputs ==1
    ...
    acc_en_in = xSignal('acc_en_in');
    accumulator_en = xBlock(struct('source', str2func('hold_en_init_xblock'), 'name', 'acc_en'), ...
                        {[blk, '/acc_en'], ...
                        'hold_period', input_period/input_clk_period}, ...
                        {sync_in}, ...
                        {acc_en_in});
    for i = 1:n_stages
        acc_ins{i+1} = xSignal(['acc_in',num2str(i)]);      
        accumulators{i} = xBlock(struct('source',str2func('accumulator_hold_init_xblock'),'name',['accumulator',num2str(i)]), ...
                         {[blk, '/accumulator',num2str(i)], ...
                                'n_bits',n_bits, ...
                                'hold_period', input_period/input_clk_period, ...
                                'ext_en', 'on'}, ...
                                [acc_ins(i), {acc_en_in}], ...
    ...
    downsampler = xBlock(struct('source', str2func('downsample_init_xblock'), 'name', 'Down_sample1'), ...
                               ...
                               'ext_en', 'on'}, ...
                          [acc_ins(n_stages+1), {acc_en_in}], ...
                          [comb_ins(1),{sync_out}]); 
                      
    comb_en_in = xSignal('comb_en_in');
    comb_en = xBlock(struct('source', str2func('hold_en_init_xblock'), 'name', 'comb_en'), ...
                        {[blk, '/comb_en'], ...
                        'hold_period', 2*(input_period/input_clk_period)}, ...
                        {sync_in}, ...
                        {comb_en_in});                  
    for i = 1:n_stages
        ...
        combs_sub{i} = xBlock(struct('source', 'AddSub', 'name', ['comb_sub_ins', num2str(i)]), ...
                                struct('mode', 'Subtraction', ...
                                       ...
                                       'en', 'on'), ...
                                  [comb_ins(i),comb_sub_ins(i), {comb_en_in}], ...
                                  comb_ins(i+1));
...
if ~isempty(blk) && ~strcmp(blk(1),'/')
    clean_blocks(blk);
    fmtstr=sprintf('recursive\n%d_%din\ndec_rate = 2\nclock(actualy/effective): %d/%d',input_bits, bin_pt, input_clk_period, input_period);
