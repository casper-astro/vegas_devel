function pfb_improved_draw(pfb_size, n_sim_inputs,  n_taps , data_n_bits, coeff_n_bits, output_bit_width, window_fn, bram_latency_coeff, bram_latency_delay, bin_width, end_scale_val, endDelay, cheap_sync, register_delay_counter, autoplace_mode, autoplace_optimize)
...
%         
    ...
    bLastTapA.bindPort({sDelayOut{2*(n_taps-1)-1}, sTapACoeffOutPrevious, sTapADataOutPrevious}, {xSignal});
...
if( cheap_sync == 1)
    ...
    sSyncOut = xDelay(sGenSync, endDelay + n_taps + 2, 'sync_delay_end');
